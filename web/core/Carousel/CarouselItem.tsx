  ...
  `
    ...
    const getVariantBody = () => {
      switch (variant) {
        case 'richTextBelow': {
          return (
            <figure
              className={`flex h-full flex-col ${displayMode === 'single' ? `${singleWidths}` : ''}`}
            >
              <div
                className={`${
              ...
              >
                <div
                  className={`flex h-fit w-full max-w-text flex-col gap-2 last:self-end`}
                >
                  {getTitleElement()}
                  {getContentElement()}
                  {attribution && (
    ...
    return (
      <li
        ...
        className={twMerge(
          `relative mt-1 focus:outline-hidden focus-visible:outline-dashed focus-visible:outline-2 focus-visible:outline-grey-50 focus-visible:outline-offset-2 dark:focus-visible:outline-white-100 ${displayMode === 'single' ? singleVariantClassNames : scrollVariantClassNames}`,
          className,
        )}
      >
        {getVariantBody()}
