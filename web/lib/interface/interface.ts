...
export const getTranslations = async () => {
  ...
  try {
    const results = await client.fetch(groq`  *[_type == "textSnippet"]`)
    return {
      isSuccess: true,
      data: results,
    }
...
const getExternalRedirects = async () => {
  const result = await notSecuredTokenClient.fetch(`*[_type == "externalRedirect"]{from,to}`)
  const externalRedirects = result
    .filter((e: any) => e)
    .map((externalRedirect: any) => {
      return {
