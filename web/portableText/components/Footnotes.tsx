...
function Footnotes({ blocks }: { blocks: PortableTextBlock[] }) {
  ...
  return (
    <footer
      role='doc-endnotes'
      data-prose='medium'
      className='border-autumn-storm-40 border-y py-2 text-sm italic lg:text-xs'
    >
        ...
        {notes.map(({ _key, text }: { _key: any; text: any }) => (
          ...
          <li id={`${_key}`} key={_key}>
            <p className='m-0 p-0'>
              ...
              <BaseLink
                href={`#${`back_ref_${_key}`}`}
                className="relative h-fit w-fit px-2 font-thin text-sm before:absolute before:top-1/2 before:left-1/2 before:aspect-square before:w-12 before:translate-x-[-50%] before:translate-y-[-50%] before:cursor-pointer before:rounded-[100%] before:content-[''] hover:no-underline lg:text-2xs"
