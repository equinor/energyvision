...
const assetLibraryItems = (S) =>
  [
    ...
    Flags.IS_DEV && S.divider(),
    Flags.IS_DEV && UnusedAssetFilters(S),
  ].filter((e) => e)

export const AssetLibrary = (S) =>
  S.listItem()
    .title('Asset library')
    .icon(LibraryIcon)
    .child(S.list('assets').id('assets').title('Asset library').items(assetLibraryItems(S)))
