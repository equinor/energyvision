import { pipe } from 'fp-ts/lib/function'
import { ap } from 'fp-ts/lib/Identity'
import * as A from 'fp-ts/lib/Array'
import * as O from 'fp-ts/lib/Option'
import { NewsArticle } from './sanity'
import { NewsIndex } from './algolia'

type MappableObjectType = {
  _key: string
  title: string
  ingress: string
  text: string
}

type MapperFunctionType = (page: NewsArticle) => (obj: MappableObjectType) => NewsIndex
const mapperFunction: MapperFunctionType =
  (page) =>
  ({ _key, title, ingress, text }) => ({
    slug: page.slug,
    objectID: `${page._id}- ${_key}`,
    type: 'page',
    pageTitle: page.title,
    title,
    ingress,
    text,
  })

// TODO: change for news article
type MapDataType = (page: NewsArticle) => NewsIndex[]
export const mapData: MapDataType = (page) =>
  pipe(
    mapperFunction,
    ap(page),
    (fn) =>
      pipe(
        pipe(
          O.fromNullable(page.textBlocks),
          O.map(A.map(fn)),
          O.getOrElse(() => [] as NewsIndex[]),
        ),
        A.concat(
          pipe(
            O.fromNullable(page.accordions),
            O.map(A.map(fn)),
            O.getOrElse(() => [] as NewsIndex[]),
          ),
        ),
      ),
  )

