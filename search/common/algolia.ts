...
const algoliaSearchCurried = (appId: string) => (apiKey: string) =>
...
type UpdateIndexType = (
  ...
  index: string,
) => (client: Algoliasearch) => TE.TaskEither<Error, BatchResponse[]>;
  ...
  pipe(
    TE.tryCatch(
      () =>
        client.saveObjects({
          indexName: index,
          objects: data as unknown as Record<string, unknown>[],
          waitForTasks: true,
        }),
      (error) =>
        new Error(
          `Unable to update index. Error message: ${JSON.stringify(error)}`,
        ),
    ),
...
type DeleteIndexType = (
  slug: string,
  index: string,
) => (client: Algoliasearch) => TE.TaskEither<Error, string>;
const deleteIndex: DeleteIndexType = (slug, index) => (client) =>
  pipe(
    TE.tryCatch(
      ...
      (error) =>
        new Error(
          `Unable to delete index. Error message: ${JSON.stringify(error)}`,
        ),
    ),
...
type UpdateSettingsType = (
  ...
  index: string,
) => (client: Algoliasearch) => TE.TaskEither<Error, Algoliasearch>;
export const updateSettings: UpdateSettingsType =
  (settings, index) => (client) =>
    pipe(
      TE.tryCatch(
        () => client.setSettings({ indexName: index, indexSettings: settings }),
        (error) =>
          new Error(
            `Unable to update settings: Error message: ${JSON.stringify(error)}`,
          ),
      ),
      TE.map(() => client),
    );
...
type UpdateType = (
  ...
  mappedData: readonly IndexType[],
) => TE.TaskEither<string | Error, number[]>;
export const update: UpdateType =
  (indexName) => (indexSettings) => (mappedData) =>
    pipe(
      init(),
      TE.fromEither,
...
type DeleteType = (
  indexName: string,
) => (slug: string) => TE.TaskEither<string | Error, string>;
...
type GenerateIndexNameType = (
  identifier: string,
) => (language: string) => (environment: string) => string;
export const generateIndexName: GenerateIndexNameType =
  (identifier) => (language) => (environment) =>
    pipe(
      environment,
      (env) => `${env.toLowerCase()}_${identifier}_${language}`,
    );
